...
## Effect size for collective identification. 
cohens_d(data$CI_difference, mu = 0)
...
## Effect size for dem and rep differences in CI
cohens_d(data$CI_difference ~ data$pol_id)
...
ggplot(CI_data, aes(x = CI_difference, y = fairness)) + ## , fill = pol_id, colour = pol_id)) +
  ...
  labs(x = "Collective Identification",
       ...
       title = "Experiment 2 - Political Groups",
...
cohens_d(study1$CI_difference, study2$CI_difference)
...
## Effect size 
F_to_eta2(f = c(11.693, 1.843, 0.590), df = c(1,1,1), df_error = c(581, 581, 581), ci = .90, alternative = "greater")
...
p1 <- ggplot(data_subset, aes(x = condition.f, y = fairness)) + 
  ...
  theme_bw() +
  labs(title = "Experiment 2 - Political Groups",
              x = "Condition",
              y = "Fairness Ratings", 
              color = "Condition",
              fill = "Condition") + 
...
p2 <- ggplot(study1, aes(x = condition.f, y = fairness)) + 
  ...
  theme_bw() +
  labs(title = "Experiment 1 - Minimal Groups",
       x = "Condition",
       y = "Fairness Ratings", 
       color = "Condition",
       fill = "Condition") + 
